...
/** The placeholder that ships in the docs. Never a valid runtime secret. */
...
// ── Fail closed ────────────────────────────────────────────────────────
// The secret is the only thing standing between a stranger and a write
// channel into a live Claude Code session, and it used to default to a value
// printed in this repo's README. A forgotten env var was therefore enough to
// put a listener on 0.0.0.0 with a publicly known password.
//
// So there is no default any more. Rather than exiting — which would break
// zero-config introspection, the reason the Dockerfile exists — an unpaired
// intercom runs MCP-only: the tools are still listed, but nothing binds a
...
/** What to tell Claude — and through it the user — when the tools can't work. */
